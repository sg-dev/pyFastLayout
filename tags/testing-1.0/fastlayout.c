...
void layout_fr( int *es, int edges, int dummy_dim, float *position, int nodes, int dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
...
void layout_fr_2x2( int *es, int edges, int dummy_dim, float *position, int nodes, int dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
...
// - problem: non working swig interface
void layout_fr_stride(float *position, int nodes, int *es, int edges, int dummy_dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
...
* - temp:    some sort of temperature 
*/
void layout_fr_loop_reorder( int *es, int edges, int dummy_dim, float *position, int nodes, int dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
...
* - temp:    some sort of temperature 
*/
void layout_fr_block_iloop2( int *es, int edges, int dummy_dim, float *position, int nodes, int dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
...
void layout_fr_omp( int *es, int edges, int dummy_dim, 
  ...
  struct timeval start, end;
  struct timeval abegin, aend, rbegin, rend;
  double atime, rtime, utime;
  atime = 0.;
  rtime = 0.;
  utime = 0.;
  ...
  for( n=0; n < MAX_IT; ++n ) {
    ...
    atime += gettime( abegin, aend );
    
    // repulsive forces
    gettimeofday(&rbegin, NULL);
    #pragma omp parallel shared(position, dplx, dply, temp, nodes, dim) private(i,j, dx, dy, len, dxi, dyi, px, py)
    { 
  ...
  gettimeofday(&end, NULL);
  double time_elapsed = gettime(start, end);
  printf("Layouting this graph took %1.2f sec.\n Time for attractive forces: %1.2f\n Time for repulsive forces: %1.2f\n Time for update: %1.2f\n", time_elapsed, atime, rtime, utime);
}
...
void layout_fr_omp_simd( int *es, int edges, int dummy_dim, 
  ...
  for( n=0; n < MAX_IT; ++n ) {
    ...
    // repulsive forces
    gettimeofday(&rbegin, NULL);
    #pragma omp parallel shared(position, dplx, dply, temp, nodes, dim) private(i,j, dx, dy, len, len2, dx2, dy2, pxi, pyi, dxi, dyi, px, py, di, pi, pj, d, l)
    { 
      float *dx_local = space;
      float *dy_local = space + nodes;
      for(i = 0; i < nodes; ++i ) {
...
void layout_fr_double_simd( int *es, int edges, int dummy_dim, 
                            ...
                            int dim, int MAX_IT, double temp ) {
  struct timeval start, end;
  struct timeval rstart, rend, astart, aend, ustart, uend;
  double rtime, atime, utime;
  rtime = atime = utime = 0.;
  gettimeofday(&start, NULL);
  ...
  gettimeofday(&end, NULL);
  double time_elapsed = gettime(start, end);
  printf("Layouting this graph took %1.4f sec.\n\trepulsive part: %1.4f \n\tattractive part: %1.4f\n\tupdate part: %1.4f\n", time_elapsed, rtime, atime, utime);
}
...
// - idea: jloop by 4
void layout_fr_simd_1x4( int *es, int edges, int dummy_dim, float *position, int nodes, int dim, int MAX_IT, float temp ) {
  struct timeval start, end;
  gettimeofday(&start, NULL);
